...
public class DishController {
    ...
    public R<Page> page(int page, int pageSize, String name) {
        ...
        //对象拷贝
        BeanUtils.copyProperties(pageInfo, dishDtoPage, "records");//忽略records 之外 其它的字段
        ...
        List<DishDto> list = records.stream().map((item) -> {
            ...
            BeanUtils.copyProperties(item, dishDto);  //把item中的属性copy 到dishDto 中
...
//        redisTemplate.delete(keys);
     ...
     * 根据条件查询对应的菜品数据
     * 使用Redis 缓存菜品数据
     * 1.改造DishController 的list 方法，先从redis 中获取菜品数据，如果有则直接返回，无需查询sql，如果没有则查sql,并将查询到的菜品数据放入redis中
     * 2.改造DishController 的save 和 update 方法，加入清理缓存的逻辑
     *
     * @param dish
     * @return
     */
